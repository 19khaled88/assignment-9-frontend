...
import {
  useAllGameTypesQuery,
  useCreateGameTypeMutation,
} from "@/redux/api/gameTypeApi";
...
const GameType = () => {
  ...
  //create field
  const [createGameType, { error: fieldCreateError, isLoading }] =
    useCreateGameTypeMutation();
  ...
  const handleChange = (event: any) => {
    ...
    setValues({
      ...inputsValue,
      [name]:
        name === "numberOfPalyers" && value != "" ? parseInt(value) : value,
      // [name]:value
    });
  };
  ...
  const submitHandler = async () => {
    ...
    try {
      ...
      const fetched = await fetch(
        "https://api.cloudinary.com/v1_1/be-fresh-ltd/image/upload",
        {
          method: "POST",
          body: formData,
        }
  ...
  return (
    <div className="relative">
      <div className="flex flex-row justify-between items-center">
        <p className="text-2xl text-gray-600">Game Type List</p>
        <Button
          className=""
          onClick={() => handleAddField("my_modal_2")}
          type="primary"
        >
          Add Game Type
        </Button>
        ...
        >
          ...
          <div>
            ...
            <input
              type="number"
              onChange={handleChange}
              value={inputsValue["numberOfPalyers"]}
              id="small-input"
              name="numberOfPalyers"
              className={` ${
                emptyError.errorName === "numberOfPalyers"
                  ? "border border-red-400"
                  : "border border-gray-400"
              } block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            />
