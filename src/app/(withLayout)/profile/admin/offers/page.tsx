...
const Offers = () => {
  ...
  //all gametypes
  const { data: allGameType, isLoading: gameTypeLoading } =
    useAllGameTypesQuery({ ...query });

  //offer list
  const { data: offerData } = useAllOffersQuery({ ...query });

  //turf list
  const { data: allTurf, isLoading: turfLoading } = useAllTurfsQuery({
    ...query,
  });

  //field with turf id
  const { data: fieldsData, isLoading: fieldLoading } = useAllFieldsQuery({
    ...query,
  });

  const findSingleField = () => {
    const res = fieldsData?.data?.data.filter(
      (item: any) => item.turfId === inputsValue["turfId"]
    );
    let array: any = [];
  ...
  //create offer
  const [createOffers, { error: fieldCreateError, isLoading }] =
    useCreateOffersMutation();
  ...
  const handleChange = (event: any) => {
    ...
    setValues({
      ...inputsValue,
      [name]:
        name === "price_per_hour" && value != "" ? parseInt(value) : value,
    });
  };
  ...
  return (
    <div className="relative">
      <div className="flex flex-row justify-between items-center">
        <p className="text-2xl text-gray-600">Game offer List</p>
        <Button
          className=""
          onClick={() => handleAddOffer("my_modal_2")}
          type="primary"
        >
          Add Offers
        </Button>
      ...
      <dialog
        id="my_modal_2"
        className={`${
          hideForm === true ? "hidden" : "visible"
        } modal mx-auto my-auto rounded-md shadow-2xl`}
        style={{ width: "500px" }}
      >
        <div className="modal-box">
          <h3 className="font-bold text-lg text-center pt-10 ">
            Insert Offers
          </h3>
        </div>
        <form
          method="dialog"
          id="cleanFormData"
          className="modal-backdrop flex flex-col mx-auto w-2/3 gap-2 pb-10"
        >
          <div>
            <label
              htmlFor="small-input"
            ...
            <select
              value={inputsValue["turfId"]}
              onChange={handleChange}
              name="turfId"
              className={` ${
                emptyError.errorName === "turfId"
          ...
          <div>
            ...
            <select
              value={inputsValue["fieldId"]}
              onChange={handleChange}
              name="fieldId"
              className={` ${
                emptyError.errorName === "fieldId"
                  ? "border border-red-400"
                  : "border border-gray-400"
              } block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            >
          ...
          <div>
            ...
            <select
              value={inputsValue["gameTypeId"]}
              onChange={handleChange}
              name="gameTypeId"
              className={` ${
                emptyError.errorName === "gameTypeId"
                  ? "border border-red-400"
                  : "border border-gray-400"
              } block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            >
          ...
          <div>
            ...
            <input
              type="text"
              onChange={handleChange}
              value={inputsValue["price_per_hour"]}
              id="small-input"
              name="price_per_hour"
              className={` ${
                emptyError.errorName === "price_per_hour"
                  ? "border border-red-400"
                  : "border border-gray-400"
              } block w-full p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 sm:text-xs focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            />
          ...
          <div className="flex flex-row justify-between">
            <button
              ...
              type="button"
              className="btn bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-md w-1/2 text-white"
            >
              Submit
            </button>
            <button onClick={() => closeAndCleanForm()} className="btn">
              Close
            </button>
          </div>
        </form>
