    ...
    gameTypes != undefined &&
      gameTypes?.data.data.map((item: any, index: number) => {
        array.push(
          <div
            key={index}
            className={`${styles.hoverItem} card w-full bg-base-100 shadow-xl`}
          >
            <img
            ...
            <div className="card-body p-4">
              <h2 className="card-title">
                <div className="badge badge-secondary text-2xl font-bold text-gray-500">
                  {item.name}
                </div>
              </h2>
              ...
              <div className="card-actions justify-end">
                <div className="badge badge-outline">
                  No of Players : {item.numberOfPalyers}
                </div>

                <Link
                  href={{
                    pathname: "single",
                    query: { id: item.id, group: "gameType" },
                  }}
                  // href={`/single/${item.id}`}
    ...
    return (
      <div className="flex flex-col pt-10 justify-items-center justify-center ">
        <h1 className="text-center py-5 text-2xl font-bold text-gray-500">
          Game, What you can play in our trufs
        </h1>
      ...
      <div className="flex flex-col pt-10 justify-items-center justify-center relative">
        ...
        <Space className="my-6">
          <Space.Compact className="absolute inset-x-0 w-3/4 md:w-2/3  lg:w-1/2 mx-auto xl:w-1/3 xl:mr-10 top-24">
            ...
            />
            <Input
              // defaultValue="Search for game type"
              placeholder="Search by game name, or player's number"
